...
WITH versions(name, version) AS (
    ...
    WHERE
        name IN ('timescaledb', 'postgis', 'pg_prometheus', 'hotforge', 'timescale_prometheus_extra', 'promscale', 'timescaledb_toolkit', 'timescale_analytics', 'timescaledb_cloudutils')
    UNION ALL
    SELECT
        'postgresql.version',
